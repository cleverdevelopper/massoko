...
export default function ConversationsScreen() {
  ...
  useEffect(() => {
    ...
    const onReceiveMessage = (msg: any) => {
      setConversations(prev => {
        const conversationIndex = prev.findIndex(c => c.id == msg.conversation_id);
        ...
        if (conversationIndex !== -1) {
          ...
          const updatedConv: Conversation = {
            ...
            time: msg.sent_at ? new Date(msg.sent_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    const onUserTyping = (data: any) => {
      if (data.user_id !== currentUser.id) {
        setConversations(prev => prev.map(c => 
          c.id == data.conversation_id ? { ...c, isTyping: true } : c
        ));
      }
    };
    ...
    const onUserStopTyping = (data: any) => {
      setConversations(prev => prev.map(c => 
        c.id == data.conversation_id ? { ...c, isTyping: false } : c
      ));
    };
    ...
    return () => {
      ...
      // We don't leave rooms here as we want to keep receiving events while on Home
    };
  ...
  const renderChatItem = (item: Conversation) => {
    const localName = contactMap[normalizePhone(item.phone)] || item.name;
    return (
      <TouchableOpacity 
        key={item.id}
        ...
        onPress={() => router.push({
          pathname: '/chat/[id]',
          params: { id: String(item.id), name: localName, image: item.image || '', partnerId: String(item.other_user_id) }
        })}
      >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
              ...
              {item.isTyping ? (
                ...
                <Text style={styles.chatMessage} numberOfLines={1}>
                  {(item.last_message && /^[1-3]:/.test(item.last_message)) ? '🔒' : item.last_message}
                </Text>
              )}
            </View>
            {item.unread > 0 && (
...
const styles = StyleSheet.create({
  ...
  typingText: { fontSize: 15, color: '#8E8E93', fontStyle: 'italic' },
